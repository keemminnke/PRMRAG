#!/usr/bin/env python3
"""Evaluate with FlashRAG — Direct generation without RAG (no retrieval).
...
def parse_args():
    ...
    p.add_argument("--tag", type=str, default="direct")
...
def main():
    ...
    # Dummy retriever config (needed for SequentialPipeline init, but naive_run skips it)
    # We use a minimal config and override to use naive_run
    config_dict = {
        ...
        # Retriever (dummy — not actually used in naive_run)
    ...
    print("=" * 70)
    print("FlashRAG Direct (No RAG) Evaluation")
    print("=" * 70)
    ...
    # Use naive_run = direct generation without retrieval
    result_dataset = pipeline.naive_run(test_data, do_eval=True)
